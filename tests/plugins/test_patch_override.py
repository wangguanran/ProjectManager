...
class TestPatchOverride:
    ...
    def test_po_apply_basic_success(self):
        ...
        projects_info = {
            "test_project": {"board_name": "test_board", "PROJECT_PO_CONFIG": "po1"}
        }
        project_name = "test_project"

        with patch("os.path.join") as mock_join, patch(
            "os.path.isdir"
        ) as mock_isdir, patch("src.plugins.patch_override.log") as mock_log:
            ...
            mock_log.info.assert_any_call(
                "start po_apply for project: '%s'", project_name
            )
            mock_log.info.assert_any_call(
                "po apply finished for project: '%s'", project_name
            )
    ...
    def test_po_apply_missing_board_name(self):
        ...
        with patch("src.plugins.patch_override.log") as mock_log:
            ...
            mock_log.error.assert_called_with(
                "Cannot find board name for project: '%s'", project_name
            )
    ...
    def test_po_apply_empty_po_config(self):
        ...
        with patch("src.plugins.patch_override.log") as mock_log:
            ...
            mock_log.warning.assert_called_with(
                "No PROJECT_PO_CONFIG found for '%s'", project_name
            )
...
class TestPatchOverrideRevert:
    ...
    def test_po_revert_basic_success(self):
        ...
        with patch("os.path.join") as mock_join, patch(
            ...
            assert result is True
            mock_log.info.assert_any_call(
                "start po_revert for project: '%s'", project_name
            )
            mock_log.info.assert_any_call(
                "po revert finished for project: '%s'", project_name
            )
...
class TestPatchOverrideNew:
    ...
    def test_po_new_invalid_name_format(self):
        ...
        with patch("src.plugins.patch_override.log") as mock_log:
            # Act
            result = self.PatchOverride.po_new(
                env, projects_info, project_name, po_name, force=True
            )
    ...
    def test_po_new_missing_board_info(self):
        ...
        with patch("src.plugins.patch_override.log") as mock_log:
            ...
            assert result is False
            mock_log.error.assert_called_with(
                "Board info missing for project '%s'", project_name
            )
...
class TestPatchOverrideDelete:
    ...
    def test_po_del_invalid_name_format(self):
        ...
        with patch("src.plugins.patch_override.log") as mock_log:
            # Act
            result = self.PatchOverride.po_del(
                env, projects_info, project_name, po_name, force=True
            )
...
class TestPatchOverrideList:
    ...
    def test_po_list_no_po_directory(self):
        ...
        with patch("os.path.join") as mock_join, patch(
            ...
            assert not result
            mock_log.warning.assert_called_with(
                "No po directory found for '%s'", project_name
            )
    ...
    def test_po_list_empty_config(self):
        ...
        with patch("os.path.join") as mock_join, patch(
            ...
            mock_log.info.assert_called_with(
                "start po_list for project: '%s'", project_name
            )
