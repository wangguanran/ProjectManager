"""
Base test class for patch override tests to reduce code duplication.
"""
...
class BasePatchOverrideTest(ABC):
    """Base test class for patch override operations to reduce code duplication."""
    ...
    def setup_method(self):
        ...
        self.patch_override = self.patch_override_cls(
...
"""
        with open(
            os.path.join(patches_dir, "main.py.patch"), "w", encoding="utf-8"
        ) as f:
            f.write(patch_content)

        # Create an override file
        with open(
            os.path.join(overrides_dir, "new_file.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(overrides_dir2, "config.txt"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(patches_dir3, "main.py.patch"), "w", encoding="utf-8"
        ) as f:
            f.write("This patch should be excluded\n")
        with open(
            os.path.join(overrides_dir3, "config.txt"), "w", encoding="utf-8"
        ) as f:
    ...
    def _change_to_test_repo(self):
        """Change to test repository directory and return original directory."""
        original_cwd = os.getcwd()
        os.chdir(self.test_repo_path)
        return original_cwd

    def _restore_original_dir(self, original_cwd):
        """Restore original directory."""
        os.chdir(original_cwd)

    @abstractmethod
    def test_basic_functionality(self):
        """Test basic functionality - to be implemented by subclasses."""
        return True
