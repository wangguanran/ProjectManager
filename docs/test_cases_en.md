...
| CLI-010 | CLI/Arg Parsing | `update --dry-run` works without projects dir (upgrade alias kept) | Run in an empty non-project workspace | 1. Run `python -m src update --dry-run`.<br>2. Run `python -m src update --install-kind package --dry-run`.<br>3. Check stderr output. | Both commands exit 0 and do not print `common config not found` / `projects directory does not exist` warnings. The explicit package dry-run reports `install_kind=package`. | P1 | Functional |
...
| CLI-019 | CLI/Execution UI | Interactive TTY defaults to Rich BuildKit-style progress output | Dataset A completed in an interactive terminal | 1. Run `python -m src project_build projA --dry-run --no-po --no-diff` in a real TTY.<br>2. Observe stdout while the command is running. | Output stays in a single terminal pane and uses a BuildKit-style summary such as `[+] project build: projA ...`, followed by `=> [i/N] ...` step lines. When inner commands are active or there is spare terminal height, nested `=> => RUN ...` / `DONE ...` detail rows are shown for the current or recent costly actions; when the terminal height is constrained, lower-priority detail rows are automatically folded before the top-level step rows. The command does not switch into the full-screen Textual UI unless `--output=tui` is explicitly provided. | P1 | DX |
| CLI-019a | CLI/Execution UI | Non-interactive runs fall back to plain raw progress output | Dataset A completed | 1. Run `python -m src project_build projA --dry-run --no-po --no-diff` from a non-interactive stdout context (for example via subprocess capture or shell redirection).<br>2. Inspect stdout. | Output uses stable numbered progress lines such as `#1 [project build: projA]` and `#1 DONE 0.00s`, with no full-screen TUI control codes. | P1 | DX |
| CLI-019b | CLI/Execution UI | `--output=raw` bypasses the TUI and uses docker-style progress output | Dataset A completed | 1. Run `python -m src po_apply projA --output=raw`.<br>2. Inspect stdout/stderr. | Output uses numbered progress lines like `#1 [po apply: projA]`, per-step logs prefixed with the same step number, and `DONE` / `FAILED` summaries; it does not include `STEP_*`, `COMMAND_*`, `SESSION_END`, or file/function logger metadata. | P1 | DX |
| CLI-019c | CLI/Execution UI | Remote Linux real-project verification shows Rich BuildKit-style progress | Remote Linux `192.168.0.97:2204` available; workspace `/home1/wangguanran/workspace/MT5825_A14`; `~/.local/bin/projman` points to the current beta binary | 1. Open a real TTY on the remote Linux host.<br>2. Run `cd /home1/wangguanran/workspace/MT5825_A14 && export PATH="$HOME/.local/bin:$PATH"`.<br>3. Optionally run `projman po_revert MT5825` to clean the workspace if revert is currently possible.<br>4. Run `projman po_apply MT5825` and observe stdout while it is running. | Output stays in the same terminal pane and uses the Rich BuildKit-style renderer, including a summary like `[+] po apply: MT5825 ...`, numbered step lines such as `=> [i/N] ...`, and nested `=> => RUN ...` / `DONE ...` detail rows that expose the current or recent costly repo-level actions while there is spare terminal height. On shorter terminals, those nested detail rows are folded automatically while the top-level step rows remain visible. The command returns to the shell prompt after `FINISHED` and does not switch into the full-screen Textual UI unless `--output=tui` is explicitly provided. | P1 | DX |
| CLI-019d | CLI/Execution UI | `--output=logger` bypasses the execution UI and uses standard logger lines | Dataset A completed | 1. Run `python -m src po_apply projA --output=logger`.<br>2. Inspect stdout/stderr. | Output uses the standard colored logger line format such as `[2026-04-16 11:27:30,247] [INFO    ] [__main__.py             ] [main                    ] [1130] ...`; it does not include BuildKit summaries like `[+] ...`, numbered raw progress lines like `#1 ...`, or the full-screen TUI. | P1 | DX |
...
| CLI-021 | CLI/Execution UI | Successful explicit TUI waits for Enter before exit | Dataset A completed in an interactive terminal | 1. Run a successful human-facing command such as `python -m src po_apply projA --output=tui` in a real TTY.<br>2. Wait for execution to finish.<br>3. Observe the completed TUI, then press `Enter`. | The full-screen TUI remains visible after success, shows a completion hint, and exits only after `Enter` is pressed. | P2 | DX |
| CLI-022 | CLI/Execution UI | Interactive TUI shows per-step logger output | Dataset A completed in an interactive terminal | 1. Run a command that emits `log.info(...)` during step execution, such as `python -m src po_apply projA`.<br>2. Select a child step after logs are emitted.<br>3. Inspect the right-side log pane. | The selected step shows the logger lines emitted during that step, not only command stdout/stderr. | P2 | DX |
| CLI-023 | Packaging/Release | Release binary checks reject non-standalone artifacts on every platform | Platform-specific CI runner available | 1. Build the release binary for the runner platform.<br>2. Run `python scripts/check_linux_standalone_binary.py "$(cat out/projman_binary_path.txt)"`.<br>3. Inspect workflow logs for the same check in release build jobs. | Linux binaries must not declare external shared-library dependencies; macOS binaries may only link system dynamic libraries/frameworks; Windows binaries may only import allowed system DLLs. CI fails otherwise. | P0 | Release |
| CLI-023a | Packaging/Release | Build artifacts include both Python packages and standalone binaries | Platform-specific CI runner available | 1. Run `./build.sh`.<br>2. Inspect `out/package/` and `out/binary/`.<br>3. Check `out/projman_wheel_path.txt` and `out/projman_binary_path.txt`. | `out/package/` contains at least one wheel and one sdist, `out/binary/` contains the standalone binary, and the path marker files point at the latest wheel/binary artifacts. | P1 | Release |
| CLI-023b | Packaging/Release | Default install flow uses the managed package runtime while binary install stays available | Platform-specific CI runner available with Python `venv` support | 1. Run `./build.sh`.<br>2. Run `./install.sh --user` and then `~/.local/bin/projman --version`.<br>3. Remove `~/.local/bin/projman` and `~/.local/lib/projman`.<br>4. Run `./install.sh --binary --user` and then `~/.local/bin/projman --version`. | The default install creates a launcher in `~/.local/bin/projman` backed by a managed runtime under `~/.local/lib/projman/venv`. The explicit binary install still places the standalone onefile launcher at `~/.local/bin/projman`. Both invocations complete successfully. | P1 | Release |
...
| PO-005c | PO Apply | Commit apply skips when patch SHA already exists in history | Commit patch prepared under `po/<po>/commits/` via `git format-patch`, and the source commit is still in target repo history | 1. Create a commit in target repo.<br>2. Export it to `projects/<board>/po/<po>/commits/`.<br>3. Keep the commit in repo history.<br>4. Run `python -m src po_apply projA`. | The commit patch is skipped before `git am`; applied record marks it as already in history and stores the original commit SHA. | P1 | Functional |
...
| PO-026 | PO Create | po_new Textual selection (`--tui` alias) | Workspace has modified/deleted files | 1. Run `python -m src po_new projA po_tui --tui`.<br>2. Use the Textual file picker to select files.<br>3. Choose action (patch/override/remove or skip) and confirm. | The Textual picker is shown; the preview/details pane updates before writing; selected files are processed accordingly. | P2 | DX |
| PO-027 | PO Create | po_new TUI fallback stays clean when Textual cannot start | Run in an env where the Textual renderer cannot start | 1. Run `python -m src po_new projA po_tui --tui`.<br>2. Observe output. | Command does not crash or print a stack trace; it falls back to the standard non-full-screen flow (`--output=raw` / console path) and still reflects the underlying command result. | P2 | Negative |
...
| PLAN-002b | DX/Safety | `po_apply --emit-plan` marks commit patches skipped by existing history | Commit patch prepared under `po/<po>/commits/` via `git format-patch`, and the source commit is still in target repo history | 1. Create a commit in target repo.<br>2. Export it to `projects/<board>/po/<po>/commits/`.<br>3. Keep the commit in repo history.<br>4. Run `python -m src po_apply projA --emit-plan`.<br>5. Parse stdout as JSON. | The plan does not include `commit_apply` for that patch; it marks the action as skipped because the original commit SHA is already in history. | P1 | DX |
