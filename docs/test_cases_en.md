...
| PO-011b | PO Revert | Commit reset success | PO-005b executed | 1. Run `python -m src po_revert projA`.<br>2. Verify the commit changes are undone. | Commits from `commits/` are removed via `git reset --hard` to recorded `head_before`; no revert commit is created; applied record is removed. | P1 | Functional |
