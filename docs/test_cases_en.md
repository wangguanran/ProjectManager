...
| CLI-023 | Packaging/Release | Linux standalone binary check rejects shared-library dependencies | Linux environment with `readelf` and `ldd` available | 1. Run `bash build.sh` on Linux.<br>2. Run `python scripts/check_linux_standalone_binary.py out/binary/projman`.<br>3. Inspect workflow logs for the same check in Linux build jobs. | The check passes only when the Linux binary is a single standalone artifact with no external shared-library dependencies; CI fails otherwise. | P0 | Release |
