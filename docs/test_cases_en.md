...
| CLI-019b | CLI/Execution UI | `--raw-output` bypasses step events and uses plain logger output | Dataset A completed | 1. Run `python -m src po_apply projA --raw-output`.<br>2. Inspect stdout/stderr. | Output contains only the standard `[timestamp] [LEVEL] [file] [func] [line] message` logger lines and does not include `STEP_*`, `COMMAND_*`, `SESSION_END`, or `LOG_*` wrappers. | P1 | DX |
