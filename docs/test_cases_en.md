...
| CLI-019b | CLI/Execution UI | `--raw-output` routes logger messages through structured `LOG_*` events only | Dataset A completed | 1. Run `python -m src po_apply projA --raw-output`.<br>2. Inspect stdout/stderr. | Raw mode emits structured `LOG_INFO` / `LOG_WARNING` / `LOG_ERROR` / `LOG_STDOUT` / `LOG_STDERR` events and does not print plain `[INFO]` / `[ERROR]` logger lines. | P1 | DX |
