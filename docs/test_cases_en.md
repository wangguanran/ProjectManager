...
| CLI-019b | CLI/Execution UI | `--raw-output` preserves standard logger formatting for log messages | Dataset A completed | 1. Run `python -m src po_apply projA --raw-output`.<br>2. Inspect stdout/stderr. | Raw mode keeps `STEP_*` / `SESSION_END` records line-oriented, while logger messages continue to use the standard `[timestamp] [LEVEL] [file] [func] [line] message` format instead of `LOG_*` wrappers. | P1 | DX |
...
| PO-027 | PO Create | po_new TUI fallback stays clean when Textual cannot start | Run in an env where the Textual renderer cannot start | 1. Run `python -m src po_new projA po_tui --tui`.<br>2. Observe output. | Command does not crash or print a stack trace; it falls back to the standard non-full-screen flow (`--raw-output`/console path) and still reflects the underlying command result. | P2 | Negative |
