/*
 * Generated by MTK SP DrvGen Version 03.13.6 for MT6735M. Copyright MediaTek Inc. (C) 2013.
 * Thu Nov 07 11:17:41 2019
 * Do Not Modify the File.
 */

#ifndef __CUST_EINTH
#define __CUST_EINTH
#ifdef __cplusplus
extern "C" {
#endif
#define CUST_EINTF_TRIGGER_RISING     			1    //High Polarity and Edge Sensitive
#define CUST_EINTF_TRIGGER_FALLING    			2    //Low Polarity and Edge Sensitive
#define CUST_EINTF_TRIGGER_HIGH      				4    //High Polarity and Level Sensitive
#define CUST_EINTF_TRIGGER_LOW       				8    //Low Polarity and Level Sensitive
#define CUST_EINT_DEBOUNCE_DISABLE          0
#define CUST_EINT_DEBOUNCE_ENABLE           1
//////////////////////////////////////////////////////////////////////////////


#define CUST_EINT_IRQ_NFC_NUM              0
#define CUST_EINT_IRQ_NFC_DEBOUNCE_CN      128
#define CUST_EINT_IRQ_NFC_TYPE							CUST_EINTF_TRIGGER_HIGH
#define CUST_EINT_IRQ_NFC_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_NFC_NUM              1
#define CUST_EINT_NFC_DEBOUNCE_CN      128
#define CUST_EINT_NFC_TYPE							CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_NFC_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_ENABLE

#define CUST_EINT_VOLUP_KEY_NUM              2
#define CUST_EINT_VOLUP_KEY_DEBOUNCE_CN      0
#define CUST_EINT_VOLUP_KEY_TYPE							CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_VOLUP_KEY_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_SOS_KEY_NUM              3
#define CUST_EINT_SOS_KEY_DEBOUNCE_CN      128
#define CUST_EINT_SOS_KEY_TYPE							CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_SOS_KEY_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_POWER_KEY_NUM              4
#define CUST_EINT_POWER_KEY_DEBOUNCE_CN      128
#define CUST_EINT_POWER_KEY_TYPE							CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_POWER_KEY_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_ACCDET_NUM              7
#define CUST_EINT_ACCDET_DEBOUNCE_CN      256
#define CUST_EINT_ACCDET_TYPE							CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_ACCDET_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_ENABLE

#define CUST_EINT_ALS_NUM              9
#define CUST_EINT_ALS_DEBOUNCE_CN      0
#define CUST_EINT_ALS_TYPE							CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_ALS_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_TOUCH_NUM              10
#define CUST_EINT_TOUCH_DEBOUNCE_CN      0
#define CUST_EINT_TOUCH_TYPE							CUST_EINTF_TRIGGER_FALLING
#define CUST_EINT_TOUCH_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_EXT_BUCK_OC_NUM              78
#define CUST_EINT_EXT_BUCK_OC_DEBOUNCE_CN      0
#define CUST_EINT_EXT_BUCK_OC_TYPE							CUST_EINTF_TRIGGER_HIGH
#define CUST_EINT_EXT_BUCK_OC_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE

#define CUST_EINT_PPT_KEY_NUM              90
#define CUST_EINT_PPT_KEY_DEBOUNCE_CN      128
#define CUST_EINT_PPT_KEY_TYPE							CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_PPT_KEY_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_ENABLE

#define CUST_EINT_PPT_KEY1_NUM              94
#define CUST_EINT_PPT_KEY1_DEBOUNCE_CN      128
#define CUST_EINT_PPT_KEY1_TYPE							CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_PPT_KEY1_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_ENABLE

#define CUST_EINT_GYRO_NUM              100
#define CUST_EINT_GYRO_DEBOUNCE_CN      0
#define CUST_EINT_GYRO_TYPE							CUST_EINTF_TRIGGER_LOW
#define CUST_EINT_GYRO_DEBOUNCE_EN      CUST_EINT_DEBOUNCE_DISABLE



//////////////////////////////////////////////////////////////////////////////
#ifdef __cplusplus
}
#endif
#endif //_CUST_EINT_H


