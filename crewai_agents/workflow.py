...
from crewai_agents.tools import FileUtils, GitUtils, TestUtils, TaskManager
...
class RequirementAnalystAgent(Agent):
    ...
    def __init__(self):
        super().__init__(
            ...
            "需求分析师",
            "明确用户需求，生成测试用例",
            "资深需求分析师，10年经验，擅长挖掘真实需求"
        )
        self.test_cases_file = os.path.join(
            os.path.dirname(__file__), '..', 'docs', 'test_cases_zh.md'
        )
...
class ArchitectAgent(Agent):
    ...
    def __init__(self):
        ...
        self.tasks_file = os.path.join(
            os.path.dirname(__file__), '..', 'docs', 'tasks.md'
        )
    ...
    def _process(self, task: str, context: Dict) -> str:
        ...
        return json.dumps({
            'tasks': tasks,
            'design': self._create_design(requirements)
        }, ensure_ascii=False, indent=2)
...
class ExecutorAgent(Agent):
    ...
    def _process(self, task: str, context: Dict) -> str:
        ...
        # 生成 commit message
        commit_msg = GitUtils.generate_commit_message([{
            'title': requirements,
            'status': 'done'
        }])
